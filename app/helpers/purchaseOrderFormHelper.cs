...
public static class purchaseOrderFormHelper
{
    ...
    public static bool isUpdateFormValid(changeStatusModel changeStatus)
    {
        try
        {
            if (changeStatus == null || changeStatus.purchaseOrderId <= 0 || changeStatus.currentStatusId <= 0 || changeStatus.newStatusId <= 0 || string.IsNullOrEmpty(changeStatus.comments))
                return false;
