...
function isValidForm(project) {
    try {
        if (!project.id || !project.name || !project.client || project.client.id <= 0 || !project.description || !project.startDate || !project.endDate || !project.status) {
            Swal.fire({
                title: 'Error!!',
...
$(document).ready(function() {
    ...
    $('#inStartDate').val($('#inStartDate').attr('option-selected'));
    $('#inEndDate').val($('#inEndDate').attr('option-selected'));
