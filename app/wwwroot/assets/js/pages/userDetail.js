...
async function showTraceInformation(traceId) {
    try {
        $('#loader').show();
        var response = await fetch(window.location.origin + '/user/getUserTraceById?id=' + traceId);
...
function renderJSONWithNoDifferences(afterJSON, beforeContainerSelector, afterContainerSelector) {
    var afterHTML = generateHTML(normalizeJSON(afterJSON));
