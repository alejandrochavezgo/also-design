function initializeTracesTable() {
    try {
        $('#dvTableTraces').DataTable({
            ...
            "responsive": true,
            "ajax": {
                "url": "getPurchaseOrderTracesByPurchaseOrderId",
                "type": "get",
                "data": function(d) {
                    d.id = $('#purchaseOrderId').val();
                },
...
async function showTraceInformation(traceId) {
    try {
        $('#loader').show();
        var response = await fetch(window.location.origin + '/purchaseOrder/getPurchaseOrderTraceById?id=' + traceId);
